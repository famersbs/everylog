...
const ORDERFunc = {
  [ORDER.asc]: (a: number, b: number) => a - b,
  [ORDER.desc]: (a: number, b: number) => b - a
};
...
const Book = (props: CardProps) => {
  ...
  if (logs == null) logs = [];
  logs = logs.sort((a, b) =>
    ORDERFunc[orderby](Number(a.progress), Number(b.progress))
  );
  ...
  return (
    <div>
      ...
      <div className="logs-control-bar">
        ...
        <button
          onClick={() =>
            changeOrder(orderby === ORDER.asc ? ORDER.desc : ORDER.asc)
          }
        >
          {unit} [{orderby}]
        </button>
