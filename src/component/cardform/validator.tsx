...
import { DISPLAY_DATE_TIME, InputType, FormValues, Spec } from "./type";

export default function Validator(form: FormValues, spec: [Spec]): boolean {
  ...
  for (let i = 0; i < spec.length; i++) {
    ...
    if (
      currentSpec.type === InputType.DATETIME &&
      !moment(currentValue, DISPLAY_DATE_TIME).isValid()
    ) {
      msgbox.error(
        errorTitle,
        `${currentSpec.label} is not a valid date time format`
      );
      return false;
    }
    ...
    if (currentSpec.maximum_value != null) {
      if (Number(currentValue) > Number(currentSpec.maximum_value)) {
        msgbox.error(
          errorTitle,
          `${currentSpec.label} cannot be bigger than ${
            currentSpec.maximum_value
          }`
        );
        return false;
      }
